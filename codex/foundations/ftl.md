...
type: codex-item
title: FTL
permalink: /foundations/foundations/ftl
...
##### Warp disruption
On 2027-03-08, during the _Battle of Sol_, the Federation discovered that warp travel can be disrupted using gravity generators, albeit at significant energy expense. This led to the creation of specialized ship classes using this technology to create volumes of space where warp travel is temporarily impossible: ships in the area cannot form warp tunnels to egress, and any warp tunnels that intersect the volume drop back to 3-space at the boundary. The first of these, commissioned four months after the battle, were the [VIS-1 Flycatcher-class destroyer]({{ '/foundations/space/flycatcher' | relative_url }}), which seeds a battlespace with disposable disruption probes, and the [VIS-2 Broadsword-class cruiser]({{ '/foundations/space/broadsword' | relative_url }}), which projects a smaller but continuous field of its own.
